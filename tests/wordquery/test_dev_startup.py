...
def test_dev_dictionary_path_does_not_depend_on_working_directory(tmp_path, monkeypatch):
    ...
    config.write_text(json.dumps({"database": "data/lexicon.sqlite3", "release_mode": "reviewed"}))
    monkeypatch.delenv("WORDQUERY_DB", raising=False)
    monkeypatch.delenv("WORDQUERY_RELEASE_MODE", raising=False)
